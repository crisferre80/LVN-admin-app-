-- Script corregido para tabla advertisements con columna 'placement'
-- Ejecutar este script si la tabla usa 'placement' en lugar de 'position'
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'advertisements') THEN
        ...
        RAISE NOTICE 'Tabla advertisements creada exitosamente con columna placement';
    ELSE
        -- La tabla existe, verificar y agregar columnas faltantes
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advertisements' AND column_name = 'placement') THEN
            -- Si no existe 'placement', agregar 'position' (por compatibilidad)
...
-- Usar la columna correcta (placement o position)
DO $$
BEGIN
    -- Verificar cuál columna existe
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advertisements' AND column_name = 'placement') THEN
        -- Usar placement
        INSERT INTO advertisements (title, image_url, link_url, placement, is_active)
        SELECT * FROM (VALUES
        ...
        ) AS v(title, image_url, link_url, placement, is_active)
        WHERE NOT EXISTS (SELECT 1 FROM advertisements LIMIT 1);
        RAISE NOTICE 'Datos insertados usando columna placement';
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'advertisements' AND column_name = 'position') THEN
        -- Usar position
        ...
        WHERE NOT EXISTS (SELECT 1 FROM advertisements LIMIT 1);
        RAISE NOTICE 'Datos insertados usando columna position';
    END IF;
END $$;
